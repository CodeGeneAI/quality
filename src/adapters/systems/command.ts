...
export const commandAdapter: StageAdapter<CommandAdapterOptions> = {
  ...
  async run(context) {
    ...
    const shouldSkipForFiles =
      context.files.length === 0 && context.stage.alwaysRun !== true;

    if (shouldSkipForFiles) {
      return {
        status: "skipped",
    ...
    for (const entry of normalized) {
      ...
      const suppressAllOutput =
        outputContext?.showOnSuccess === "none" &&
        outputContext.showOnFailure === "none";
      ...
      const result = await runCommand({
        ...
        onStderrChunk:
          ...
          (outputContext.showOnSuccess !== "none" ||
            ...
            : undefined,
        suppressOutput: suppressAllOutput,
      });
      ...
      const nothingOnSuccess = outputContext?.showOnSuccess === "none";
      const nothingOnFailure = outputContext?.showOnFailure === "none";
      ...
      } else if (!nothingOnSuccess && result.stdout.trim()) {
