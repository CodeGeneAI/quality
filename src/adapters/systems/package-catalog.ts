...
const loadCatalog = async (root: string, catalogPath: string): Promise<CatalogLookup> => {
  const pkg = await readJsonFile<Record<string, unknown>>(catalogPath);
  const catalogs = (pkg as { catalogs?: Record<string, Record<string, string>> })
    .catalogs ?? {};
...
export const packageCatalogAdapter: StageAdapter<PackageCatalogAdapterOptions> = {
  ...
  async run(context) {
    ...
    for (const relativePath of packagePaths) {
      ...
      const pkgJson = await readJsonFile<Record<string, unknown>>(pkgPath).catch(() =>
        undefined,
      );
      ...
      for (const section of sections) {
        ...
        for (const [depName, version] of Object.entries(
          ...
          if (catalogName && isFix) {
            (deps as Record<string, string>)[depName] = `catalog:${catalogName}`;
            updated = true;
            continue;
          }
