...
## Tests & quality gates
- Run `bun --filter @codegeneai/quality lint`, `bun --filter @codegeneai/quality typecheck`, and `bun --filter @codegeneai/quality test:unit` before committing, then follow workspace gates (`bun run test:startup` → `bun run test:int`).
